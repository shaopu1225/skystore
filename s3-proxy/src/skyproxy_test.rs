...
mod tests {
    ...
    //         );
    //         println!("-_______________________________________---------------------------------------rached here");
    ...
    #[tokio::test]
    #[serial]
    async fn test_put_then_get_with_password() {
        let proxy = setup_sky_proxy().await;
...

        {
            ...
            let req = S3Request::new(request);
            *req.password.borrow_mut() = "abc".to_string();
            let resp = proxy.put_object(req).await.unwrap().output;
            assert!(resp.e_tag.is_some());
        }
